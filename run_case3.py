...
factor=1 # 1 for case 1; 0.1 for case 2
...
for u in range(n_sim):
    ...
    beta0=cal.betagenerator(20000,20,beta01,beta02)
    X,X_total,y=cal.datagenerator(1000, 20000, 1, corst_x, rho_x, beta0,family,np.sqrt(factor),u)
    ...
    report=[int(np.ceil((100)*(i+1)-1)) for i in range(10)]   #record results in the 100,200,...,1000-th step
    summary=cal.summary(model.debeta_trajec,model.lb_trajec,model.ub_trajec,model.tao_trajec,np.arange(1000),beta0,subset,beta01,beta02)
...
np.save(os.path.join(folder_name, 'summary.npy'), np.mean(np.array(Summary),axis=0)[:,report]) 

column_names = ['beta*=0', 'beta*=1', 'beta*=-1']
row_names = ['100', '200', '300', '400', '500', '600', '700', '800', '900', '1000']
...
cal.plot1(DEBETA,LB,UB,1000,20000,folder_name)
