...
for u in range(n_sim):
    ...
    beta0=cal.betagenerator_multi_strength(500,10,beta01,beta02,beta03,beta04,beta05)
    X,X_total,y=cal.datagenerator(200, 500, 1, corst_x, rho_x, beta0,family,np.sqrt(factor),u)

    random.seed(u)
    a0 = random.sample(np.where((beta0 == 0))[0].tolist(), 2)  #pick 2 coefficients from each set
    a1 = random.sample(np.where((beta0 == beta01))[0].tolist(), 2)
    a2 = random.sample(np.where((beta0 == beta02))[0].tolist(), 2)
    a3 = random.sample(np.where((beta0 == beta03))[0].tolist(), 2)
    a4 = random.sample(np.where((beta0 == beta04))[0].tolist(), 2)
    a5 = random.sample(np.where((beta0 == beta05))[0].tolist(), 2)
    random.seed()
    subset=a0+a1+a2+a3+a4+a5
    ...
    report=[79,119,159,199]    #record results in the 80, 120,160, 200-th step
    summary=cal.summary_multi_strength(model.debeta_trajec,model.lb_trajec,model.ub_trajec,model.tao_trajec,np.arange(200),beta0,subset,beta01,beta02,beta03,beta04,beta05)
...
column_names = ['beta*=0', 'beta*=0.2', 'beta*=0.4', 'beta*=0.6', 'beta*=0.8', 'beta*=1']
