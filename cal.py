...
def betagenerator_multi_strength(p,k,beta01,beta02,beta03,beta04,beta05):
    np.random.seed(0)
    true = np.random.choice(np.arange(p),size=k,replace=False)
    np.random.seed()
    beta0 = np.zeros(p)
    true_beta = np.repeat([beta01,beta02,beta03,beta04,beta05],k//5)
...
def regularizer(x,lam,penalty):
    if penalty == "l1":
        ...
        return np.where(abs(x)<lam,(abs(x)-lam)*np.sign(x)/(3.7-1) ,0) + np.where(abs(x)<3.7*lam,(3.7*lam -abs(x))*np.sign(x)/(3.7-1),0)
    elif penalty == "mcp":
        return np.where(abs(x)<2.7*lam,np.sign(x) * ((lam-abs(x)/2.7)),0)
    else:
...
def summary_multi_strength(debeta_trajec,lb_trajec,ub_trajec,tao_trajec,out_index,beta0,subset,beta01,beta02,beta03,beta04,beta05):

    beta_real = beta0[subset]
    A0 = np.where((beta_real == 0))[0].tolist()  #groupping by true beta
    A1 = np.where((beta_real == beta01))[0].tolist()
    A2 = np.where((beta_real == beta02))[0].tolist()
    A3 = np.where((beta_real == beta03))[0].tolist()
    A4 = np.where((beta_real == beta04))[0].tolist()
    A5 = np.where((beta_real == beta05))[0].tolist()
    ...
    for o in out_index:
        ...
        for A in [A0,A1,A2,A3,A4,A5]:
            ...
            if len(A) > 0:
                cp.append(count/len(A)) #coverage probability
                bias.append(total_bias/len(A))
                length.append(total_length/len(A))
                ta.append(total_tao/len(A))
...
def plot1(DEBETA,LB,UB,NB,p,folder_name):
    T_list1 = [0] + [np.ceil(2 ** i * np.ceil(np.log(p))*4) for i in range(15)]
    xx=np.arange(NB)
    ...
    plt.savefig(os.path.join(folder_name, "result_p%s_n%s.png" % (p, NB)), dpi=600)